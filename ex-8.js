...
function sumSalesByLocation(bills) {
  const totalsByLocation = bills.reduce((acc, cur) => {
    const { location, total } = cur;
    acc.set(location, (acc.get(location) || 0) + total);
    return acc;
  }, new Map());

  const result = {};
  totalsByLocation.forEach((value, key) => {
    result[key] = value;
  });
...
const totalPaidByLocation = sumSalesByLocation(bills);
